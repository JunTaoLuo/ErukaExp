...
select parcelid, use_code, number_of_parcels, sale_price, date_of_sale, tax_year, basement, heating, air_conditioning, total_rooms, full_bath, half_bath, fireplaces, garage_type, garage_capacity, num_stories, year_built, finished_sq_ft, total_finish_area, first_floor_area, half_floor_area, finished_basement 
from cleaned.historic_sales hs where num_stories = 0
and use_code in (510, 550, 520, 401, 530, 625, 404, 470);

-- 
select parcelid, use_code, instrument_type, number_of_parcels, sale_price, date_of_sale, tax_year, basement, heating, air_conditioning, total_rooms, full_bath, half_bath, fireplaces, garage_type, garage_capacity, num_stories, year_built, finished_sq_ft, total_finish_area, first_floor_area, half_floor_area, finished_basement  
...
-- 0 deeded acreage
select parcelid, prop_class_code, class_description, property_description, deeded_acreage, sale_amount, num_parcels, market_land_value, market_impr_value, total_market_value, transfer_date, conveyance_number, deed_number, divided_flag, new_construction_flag, spec_assess_flag, foreclosure_flag, rental_registration_flag
...
-- 0 sale price
select parcelid, prop_class_code, owner_name_1, owner_name_2, previous_owner_name_1, previous_owner_name_2, class_description, property_description, deeded_acreage, sale_amount, num_parcels, market_land_value, market_impr_value, total_market_value, transfer_date, conveyance_number, deed_number, divided_flag, new_construction_flag, spec_assess_flag, foreclosure_flag, rental_registration_flag
from cleaned.monthly_tax mt
where prop_class_code in (510, 550, 520, 401, 530, 625, 404, 470)
and sale_amount = 0;
...
select count(*) from cleaned.property_transfer pt where conveyance_no is not null;
